...
function shimContent(): string {
  const header = '#!/bin/sh\n# limn — open Limn for a repository (generated by the app; reinstall via app menu if the app moves)\n'
  ...
  const normalize = [
    ...
    '  case "$1" in',
    '    -h|--help)',
    '      printf "usage: limn [DIR] [--base REF] [--branch BRANCH] [--new] [--hub]\\n"',
...
export function installCli(): { ok: boolean; message: string } {
  ...
  for (const target of targets) {
    try {
      ...
      const note = onPath
        ? `${dir} is on your PATH — run \`limn\` from any repo.`
        : `${dir} is NOT on your PATH. Add it, e.g.\n  echo 'export PATH="${dir}:$PATH"' >> ~/.zshrc`
      return { ok: true, message: `Installed limn to ${target}.\n${note}` }
    } catch {
      // try the next target
    }
  }
